...
"""Generate grad tests for trained neural network classifiers.
...
Run from the repository root:
    python tests/milestone1/base/unit/grad/generate_network_grad_tests.py
"""
...
def create_test(name, fn, inputs_data, *, tolerance=DEFAULT_TOLERANCE):
    ...
    # Compute gradients: d(sum of outputs)/d(inputs)
    primals = _forward(graph, arrays)
    ...
    for i, grad in enumerate(in_tangents):
        ...
        grad.array.astype(np.float64).tofile(test_dir / fname)
...
def build_test(
    ...
    *, n_epochs, lr, n_inputs,
    ...
    """Train a network and generate a grad test directory for it."""
